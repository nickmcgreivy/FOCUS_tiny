...
fourier_increase = 6
newp = np.append(p,
                 np.zeros(index_update(p.shape, 2, fourier_increase)),
                 axis=-1)
...
def drdtheta(p, theta):
    ...
    """
    drdtheta = np.zeros((3, p.shape[1], NS + 1))
    for m in range(p.shape[2]):
...
def vector_potential_indexed(coil_i, segment_j, ll, dl, I_arr):
    ...
    *A*: Vector potential at position r_eval
    ...
    bottom_inv = 1/bottom
    bottom_inv_cleaned = bottom_inv.at[coil_i, segment_j].set(0)
...
def total_energy(objective_array):
    ...
    # inner product is elementwise multiplication, below is faster:
    energy = np.sum(A_on_coils * dl)
    return energy
...
def toroidal_flux(objective_array, target_flux):
    ...
    """
    p, I_arr = array2both(objective_array)
    closed_l = r(p, theta)  # loop where last element equals first
    ll = closed_l[:, :-1, :]
    dl = closed_l[:, :-1, :] - closed_l[:, 1:, :]
    polint = flux_through_poloidal_loops(r_surf, ll, dl, I_arr)
...
@jit
def objective_function(objective_array):
    Q = 1.0 * quadratic_flux_error(objective_array)
    E = .0005 * total_energy(objective_array)
    L = .1 * coil_length(objective_array)
...
def array2p(objective_array):
    return np.reshape(objective_array[:p_size], p_shape)


def array2I_arr(objective_array):
    return np.reshape(objective_array[-I_arr_size:], I_arr_shape)
...
print("loss is {}".format(objective_function(objective_array)))
#print("jit_grad_func is {}".format(

res = optimize.minimize(objective_function, objective_array, jac=jit_grad_func)
print("scipy optimization leads to a loss of {}".format(res.fun))

for n in range(N):
    gradient = jit_grad_func(objective_array)

    p = p - array2p(gradient) * lr
    I_arr = I_arr - array2I_arr(gradient) * lr
    objective_array = make_array(p, I_arr)
    print(n)
    print("taking a shape step of size: {}".format(
        np.linalg.norm(array2p(gradient) * lr)))
    print("taking a current step of size: {}".format(
        np.linalg.norm(array2I_arr(gradient) * lr)))
    print("loss is {}".format(objective_function(objective_array)))
    objective_arrays.append(objective_array)
...
with tb.open_file("coils_scipy.hdf5", "w") as f:
